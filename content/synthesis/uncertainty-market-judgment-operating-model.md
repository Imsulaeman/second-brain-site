...
```mermaid
flowchart TD
    A["Idea appears"] --> B["Define the game"]
    B --> C["Taleb: alternative histories, tails, sample bias"]
    C --> D["Marks: price, risk, cycle, margin of safety"]
    D --> E["Munger: incentives, psychology, inversion, checklist"]
    E --> F{"Passes? "}
    F -->|No| G["Skip or keep as watchlist"]
    F -->|Yes| H["Size for survival"]
    H --> I["Act"]
    I --> J["Journal expected distribution and invalidation"]
    J --> K["Review process vs outcome"]
    K --> C
```
