...
Deductive reasoning starts from accepted rules or facts and applies them to a specific case to reach a conclusion that must follow if the premises are true.

A company policy states that work submitted after the deadline loses ten points. A student submits late. The conclusion follows directly: the student loses ten points. A system rule locks an account after five incorrect password attempts. The fifth failure occurs. The account locks. The movement is always the same: general rule → specific fact → necessary result.
