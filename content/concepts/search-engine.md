...
A search engine is a retrieval system that discovers, indexes, and ranks web content so users can find relevant pages through queries. Google, Bing, DuckDuckGo, Yahoo, Yandex, and similar tools are search engines. They are usually accessed through a [[concepts/web-browser|browser]], but they are not the browser itself.
...
Modern interfaces blur search with generation through AI overviews and conversational result modes. But the core identity of a search engine remains retrieval. It surfaces documents for the user to inspect, even when it adds summary layers on top.
